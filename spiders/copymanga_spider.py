...
class CopyMangaSpider:
                    ...
                    for volume in volumes:
                        ...
                        for chapter in volume_chapters:
                            ...
                            if count == 0:
                                ...
                                print('comic_name : ', title, ', chapter_name : ', chapter_name)
                                continue
                            comic_chapter_id = self.comic_chapter_db.get_comic_chapter_id_by_comic_volume_id(
                                comic_id,
                                comic_volume_id,
    ...
    def search(self, keyword=''):
        try:
            man_hua_url = self.domain + '/api/v3/search/comic'
            man_hua_response = requests.get(man_hua_url, headers={
                'Referer': self.domain,
                'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, '
                              'like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1',
                    ...
                    try:
                        ...
                        for man_hua_item in man_hua_list:
                            ...
                            for volume in volumes:
                                ...
                                self.comic_volume_db.save(comic_id, volume_name, volume_index)
                                comic_volume_id = self.comic_volume_db.get_comic_volume_id(comic_id, volume_name)
                                chapter_index = self.comic_chapter_db.get_seq_no_by_comic_volume_id(comic_id,
                                                                                                    comic_volume_id)
                                for chapter in volume_chapters:
                                    if system.global_vars.application.get_close_status() == status.YES:
                                        print('copy manga is close.')
                                    ...
                                    comic_chapter_id = self.comic_chapter_db.get_comic_chapter_id_by_comic_volume_id(
                                        ...
                                        comic_volume_id,
                                        chapter_name)
                                    chapter_url = self.domain + '/api/v3/comic/' + path_word + '/chapter2/' + chapter[
                                        'id']
                                    print(chapter_url)
                                    man_hua_chapter_response = requests.get(chapter_url, headers={
                                        'method': 'GET',
