...
class BiliNovelSpider:
                ...
                try:
                    ...
                    for xiao_shuo_item in xiao_shuo_items:
                        ...
                        for volume_item in volume_items:
                            ...
                            chapter_index = self.novel_chapter_db.get_seq_no_by_novel_volume_id(novel_id,
                                                                                                novel_volume_id)
                            for chapter_item in chapter_items:
                                if system.global_vars.application.get_close_status() == status.YES:
                                    print('bili novel is close.')
                                    return
                                chapter_index += 1
                                ...
                                time.sleep(2)
                                xiao_shuo_chapter_soup = bs4.BeautifulSoup(xiao_shuo_chapter_response_text,
                                                                           'html.parser')
    ...
    def search(self, keyword=''):
        ...
        try:
            ...
            time.sleep(5)
            element = browser.find_element(value='searchkey')
            element.send_keys(keyword)
            element.send_keys(Keys.ENTER)
            time.sleep(2)
            xiao_shuo_response_text = browser.page_source
            xiao_shuo_soup = bs4.BeautifulSoup(xiao_shuo_response_text, 'html.parser')
            book_title_items = xiao_shuo_soup.select('h1.book-title')
            if len(book_title_items) == 1 and len(book_title_items[0].text) != 0:
                print('title : ', book_title_items[0].text)
                xiao_shuo_detail_soup = xiao_shuo_soup
                xiao_shuo_detail_item = xiao_shuo_detail_soup.select('div.book-detail-info .book-layout')[0]
                title = xiao_shuo_detail_item.find('h1').text.replace('\'', '\\\'')
                cover = xiao_shuo_detail_item.find('img').get('src')
                print(cover)
